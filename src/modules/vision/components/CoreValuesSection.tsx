...
export function CoreValuesSection() {
  ...
  const startEdit = (v: CoreValue) => { setEditId(v.id); setEditName(v.name); setEditDesc(v.description ?? '') }
  ...
  const moveUp = async (idx: number) => {
    ...
    await Promise.all([updateValue(a.id, { sortOrder: b.sortOrder }), updateValue(b.id, { sortOrder: a.sortOrder })])
  }
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="mb-4 p-4 border border-gray-200 rounded-lg bg-gray-50 space-y-2">
          <input type="text" placeholder="Value name (e.g. Integrity, Growth, Family)"
            ...
            autoFocus required />
          <textarea placeholder="What this value means to you (optional)"
      ...
      {values.length === 0 ? (
        <div className="p-8 border-2 border-dashed border-gray-200 rounded-xl text-center">
          <p className="text-sm text-gray-400">No core values yet. Add the principles that guide your life.</p>
        </div>
      ) : (
              ...
              {editId === v.id ? (
                <div className="p-3 space-y-2">
                  <input value={editName} onChange={e => setEditName(e.target.value)}
                    className="w-full border border-gray-300 rounded px-2.5 py-1.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" autoFocus />
